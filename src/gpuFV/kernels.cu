...
#define VEC 1
//#define DIM 82
__global__ void gmm_2(float* invCovariances, float* logCovariances, float* logWeights, float* posteriors, int numClusters, float halfDimLog2Pi, float* data, float const* means, float infinity, int numData, int DIM) {
  int tid = threadIdx.x;
...
__global__ void kernel1_mean(float* tmpsum, float* data, float* projectionCenter, int numData, int dim) {
  int tid = threadIdx.x;
  int gid = blockIdx.x;
  extern __shared__ float tempcenter[];
  tempcenter[tid] = projectionCenter[tid];
  __syncthreads();
  for(int i=gid;i<numData;i+=gridDim.x) { //blockDim.x) {
    tmpsum[i*dim + tid] = data[i*dim+tid] - tempcenter[tid];
  }
}
