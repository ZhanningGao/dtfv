...
void gpu_pca_gzn(float* projection, float* projectionCenter, float* data, float* dst, int numData, int dimension, int dim_ori) {
  ...
  cudaMalloc((void**)&tmp2_d, numData*dimension*sizeof(float));
  cudaMalloc((void**)&tempsum_d, numData*dim_ori*sizeof(float));
  cudaMalloc((void**)&projection_d, dimension*uiWA*sizeof(float));
  cudaMalloc((void**)&projectionCenter_d, uiWA*sizeof(float));
  cudaMalloc((void**)&data_d, numData*dim_ori*sizeof(float));

  
  cudaMemcpy(projection_d, projection, dimension*dim_ori*sizeof(float), cudaMemcpyHostToDevice);
  cudaMemcpy(data_d, data, numData*dim_ori*sizeof(float), cudaMemcpyHostToDevice);
  cudaMemcpy(projectionCenter_d, projectionCenter, dim_ori*sizeof(float), cudaMemcpyHostToDevice);
  dim3 threads(dim_ori,1,1);
  dim3 blocks(2048,1,1);
  kernel1_mean<<<blocks, threads, dim_ori * sizeof(float), 0>>>(tempsum_d, data_d, projectionCenter_d, numData, dim_ori);
  cublasStatus_t ret = cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, uiHA, uiWB, uiWA,&alpha, projection_d, uiHA, tempsum_d, uiWA, &beta, tmp2_d, uiHA);

  //checkError(ret, "cublas Sgemm returned an error!\n");
  cudaMemcpy(dst, tmp2_d, dimension*numData*sizeof(float), cudaMemcpyDeviceToHost);
  cublasDestroy(handle);
  //debug---
  //float data_mean[9];
  //cudaMemcpy(data_mean, tempsum_d, dim_ori*numData*sizeof(float), cudaMemcpyDeviceToHost);
  //printf("debug --- %f,%f,%f\n", data_mean[1],data_mean[4],data_mean[6]);
  //debug---
